...
Read data & scale variables. Note we have convert data from catch per hour to catch per area in this [script](https://github.com/maxlindmark/greater-weever/blob/main/R/prepare-data/00-prep-data.qmd), to make better use of the `get_index` function in sdmTMB. Temperature data come from [GLOBAL_MULTIYEAR_PHY_001_030]( https://data.marine.copernicus.eu/product/GLOBAL_MULTIYEAR_PHY_001_030/download?dataset=cmems_mod_glo_phy_my_0.083deg_P1M-m_202311)
...
index %>% 
  ...
  geom_ribbon(aes(ymin = lwr/1000, ymax = upr/1000), alpha = 0.3, color = NA) +
  geom_line() +
  labs(x = "", y = "Biomassaindex (ton)")

ggsave(paste0(home, "/figures/fiskbarometern/Fig_3_maxlindmark.png"), dpi = 300, width = 8, height = 8, units = "cm")
```
...
## Now compare an index for the full time series (without temperature)

```{r}
#| message: false
d2 <- readr::read_csv(paste0(home, "/data/clean/trawl.csv")) %>% 
  filter(depth > 0) %>% 
...
# Read prediction grid
pred_grid2 <- readr::read_csv(paste0(home, "/data/clean/pred_grid.csv")) %>% 
  filter(depth > 0) %>% 
  mutate(depth = log(depth),
         depth_sc = (depth - mean(d$depth)) / sd(d$depth),
         depth_sq = depth_sc*depth_sc,
...
m2 <- sdmTMB(
  formula = density ~ year_f + s(depth_sc) + quarter_f,
  data = d2,
  mesh = mesh,
  family = delta_gamma(),
  spatiotemporal = list("off", "IID"),
...
pred2_q1 <- predict(m2, newdata = pred_grid2 %>% filter(quarter == 1),
                    return_tmb_object = TRUE)

pred2_q3 <- predict(m2, newdata = pred_grid2 %>% filter(quarter == 3),
                    return_tmb_object = TRUE)

index2_q1 <- get_index(pred2_q1, area = 9, bias_correct = FALSE)
index2_q3 <- get_index(pred2_q3, area = 9, bias_correct = FALSE)

index2 <- bind_rows(index2_q1 %>% mutate(quarter = as.factor(1)),
                    index2_q3 %>% mutate(quarter = as.factor(3)))

# Index
ggplot(bind_rows(index2 %>% mutate(series = "long"),
                 index %>% mutate(series = "temp")),
       aes(year, est/1000, color = series, fill = series)) +
  #geom_ribbon(aes(ymin = lwr/1000, ymax = upr/1000), alpha = 0.3, color = NA) +
  geom_line() +
  facet_wrap(~quarter, scales = "free") +
  scale_color_brewer(palette = "Set1") + 
